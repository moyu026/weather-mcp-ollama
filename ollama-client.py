    ...
    async def run(self, user_query: str):
        ...
        async with stdio_client(server=server_params) as (read_stream, write_stream):
            async with ClientSession(read_stream, write_stream) as session:
                ...
                # -------------------------------
                # 方法 1: Ollama + MCP 工具调用
                # -------------------------------
                # 第一步：询问模型是否需要工具
                # 注意：这里我们不使用 ollama.chat() 的 tools 参数，而是通过 MCP 会话手动协调。
                # 我们先让模型判断是否需要调用工具（基于工具描述）。
                ...
                if decision_text.startswith("TOOL_CALL:"):
                    try:
                        ...
                        result_with_tool["model_reply"] = final_response['message']['content']
...
async def main():
    ...
    result = await client.run("nanjing的天气怎么样") # 地点名称要用英文，中文不识别
